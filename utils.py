...
def get_mart_test_summary():
    ...
    marts = [
        'SERVICE_CATEGORIES', 'CCSR', 'CMS_CHRONIC_CONDITIONS',
        'TUVA_CHRONIC_CONDITIONS', 'CMS_HCCS', 'ED_CLASSIFICATION',
        'FINANCIAL_PMPM', 'QUALITY_MEASURES', 'READMISSION'
    ]
    ...
    for mart in marts:
        ...
        if result['total_tests'] > 0:
            passing_pct = (result['passing_tests'] / result['total_tests'] * 100).round(1)
...
def get_quality_dimension_summary():
    ...
    # Calculate passing percentages
    if not df.empty:
        df['passing_percentage'] = (df['passing_tests'] / df['total_tests'] * 100).round(1)
